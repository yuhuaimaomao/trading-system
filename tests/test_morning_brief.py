...
class TestGenerateAndSend:
    def test_full_flow(self, morning_brief):
        """完整流程：加载数据 → 调 AI → 推送"""
        with patch.object(
            morning_brief, "_load_review_report", return_value="复盘"
        ) as mock_r:
            with patch.object(
                morning_brief, "_get_macro_text", return_value="宏观"
            ) as mock_m:
                with patch.object(
                    morning_brief,
                    "_get_morning_articles",
                    return_value={"morning": {"content": "早报"}},
                ) as mock_a:
                    with patch.object(
                        morning_brief, "_get_overnight_telegraphs", return_value="电报"
                    ) as mock_t:
                        with patch.object(
                            morning_brief, "_call_ai", return_value="AI生成内容"
                        ) as mock_ai:
    ...
    def test_default_trade_date(self, morning_brief):
        ...
        with patch.object(morning_brief, "_load_review_report", return_value=""):
            with patch.object(morning_brief, "_get_macro_text", return_value=""):
                with patch.object(
                    morning_brief, "_get_morning_articles", return_value={}
                ):
                    with patch.object(
                        morning_brief, "_get_overnight_telegraphs", return_value=""
                    ):
            ...
            with patch.object(morning_brief, "_get_macro_text", return_value=""):
                with patch.object(
                    ...
                    with patch.object(
                        ...
                        with patch.object(
                            morning_brief, "_call_ai", return_value="AI"
                        ) as mock_ai:
                            with patch.object(morning_brief, "_send"):
                                morning_brief.generate_and_send("2026-05-25")
                                mock_ai.assert_called_once()
