...
class TestStrategyE2E:
    ...
    def test_market_breadth_integration(self):
        """大盘数据写入成功"""
        ...
        if data:
            ...
            assert data["market_state"] in ("普涨", "分化", "普跌", "恐慌", "连跌修复", "超跌末端")
        else:
    ...
    def test_screening_produces_candidates(self):
        ...
        ts = TrendScreener(top_n=5)
        results = ts.screen("2026-05-25")
        assert len(results) > 0

    def test_profile_builder_enriches(self):
