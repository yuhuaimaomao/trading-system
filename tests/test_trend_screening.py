...
def db_path():
    ...
    for day_offset in range(1, 21):
        d = f"2025-01-{15 - day_offset:02d}" if day_offset <= 14 else f"2024-12-{31 - (day_offset - 15):02d}"
        for code, name in [("000001", "强趋势A"), ("000002", "强趋势B")]:
            history_rows.append((
                code, name, d, 1.0, 80_0000_0000, 40_0000_0000,
                4.0, 1.3, 11.0, 10.5, 10.0, 5.0, "科技", 11.5, 11.0, 11.8, 10.8,
                11.2, 300_0000, 0.03, 100_0000, 100_0000, 3.0,
                0, 0, 0, 0, 15.0, 1.5, 10.0, 8.0,
            ))

    # 当日数据
    rows = [
        # 000001: 放量启动(vol_ratio>=1.5) + 主力介入(mf_ratio>3) → 2 tags → 场景命中
        ("000001", "强趋势A", 3.5, 100_0000_0000, 50_0000_0000,
         5.0, 1.5, 11.50, 10.80, 10.00, 8.0, "科技", 11.80,
         11.30, 11.90, 10.70, 11.50,
         500_0000, 5.0, 130_0000, 100_0000, 3.0,
         0, 0, 0, 0, 15.0, 1.5, 10.0, 8.0),
        # 000002: 放量启动 + 主力介入
        ("000002", "强趋势B", 2.0, 80_0000_0000, 40_0000_0000,
         4.0, 1.6, 12.00, 11.00, 10.00, 6.0, "金融", 12.30,
         11.80, 12.50, 11.20, 12.00,
         300_0000, 4.0, 100_0000, 100_0000, 4.0,
         0, 0, 0, 0, 12.0, 1.0, 5.0, 3.0),
        # 000003: 缩量回调(vol_ratio<=0.7, chg<=0) 不满足
        ("000003", "弱票C", 0.5, 70_0000_0000, 35_0000_0000,
         6.0, 0.8, 10.00, 10.20, 9.80, 3.0, "医药", 10.50,
         10.30, 10.80, 10.0, 10.40,
         200_0000, 2.0, 100_0000, 100_0000, 2.5,
         0, 0, 0, 0, 20.0, 2.0, 3.0, 1.0),
        # 000004: ST 股，hard_gates 直接过滤
        ("000004", "*ST失败", 0.8, 90_0000_0000, 45_0000_0000,
         7.0, 1.5, 10.50, 11.00, 10.00, 2.0, "消费", 10.90,
         10.50, 11.20, 10.30, 10.80,
         150_0000, 5.0, 100_0000, 100_0000, 3.5,
         0, 0, 0, 0, 18.0, 1.8, 6.0, 4.0),
        # 000005: 688 开头，hard_gates 直接过滤
        ("688001", "科创板", 2.0, 60_0000_0000, 30_0000_0000,
         3.5, 1.5, 10.40, 10.10, 10.00, 2.0, "半导体", 10.50,
         10.20, 10.80, 10.00, 10.30,
         100_0000, 4.0, 100_0000, 100_0000, 1.5,
         0, 0, 0, 0, 30.0, 3.0, 15.0, 12.0),
    ]
