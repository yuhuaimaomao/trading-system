...
CONTINUOUS_START = dt_time(9, 30)  # 连续竞价开始，之前不买入


class PaperTrader:
    """模拟盘自动交易器。信号买点触发自动买，止损止盈自动卖。"""

    def __init__(self, db_path: str, telegram_bot=None, portfolio: Portfolio = None):
        self.portfolio = portfolio or Portfolio(initial_cash=settings.PAPER_INITIAL_CAPITAL)
    ...
    def try_buy(
        ...
        if datetime.now().time() < CONTINUOUS_START:
            return False
        if not name or name == code:
            name = self._resolve_name(code)
        if code in self.portfolio.positions:
            return False
        if len(self.portfolio.positions) >= settings.MAX_POSITIONS:
            return self._try_swap(
                code, name, price, buy_min, buy_max, sl, tp, score, source, max_amount
            )

        # 动态仓位：max_amount 优先，否则用默认比例
        if max_amount is not None:
            capital = min(max_amount, self.portfolio.total_value * settings.DEFAULT_POSITION_PCT)
        else:
            capital = self.portfolio.total_value * settings.DEFAULT_POSITION_PCT

        volume = max(0, int(capital / price / 100) * 100)  # 防负值
        if volume < 100:
            logger.info(f"模拟盘资金不足买入 {code}")
            return False

        # 买入佣金（预留 10% 现金缓冲，防后续费用）
        cost = volume * price
        commission = max(cost * settings.DEFAULT_COMMISSION_RATE, settings.MIN_COMMISSION)
        total_cost = cost + commission
        if total_cost > self.portfolio.cash:
            safe_cash = max(0, self.portfolio.cash * 0.9 - commission)
            volume = max(0, int(safe_cash / price / 100) * 100)
            if volume < 100:
                return False
        ...
        if self.telegram:
            self.telegram.send(
                f"📝 模拟盘买入 — {code} {name}\n"
                f"   价格: {price:.2f} × {volume} 股  金额: {cost:,.0f}  佣金: {commission:.0f}\n"
    ...
    def _try_swap(
        ...
        # _ai_evaluate_swap 返回 {"sell": code, "buy": code}，
        # _rule_swap_target 返回字符串 code
        sell_code = result["sell"] if isinstance(result, dict) else result
        if not sell_code or sell_code not in self.portfolio.positions:
            logger.info(f"模拟盘换仓: 返回的卖出代码 {sell_code} 不在持仓中")
            return False
        sell_pos = self.portfolio.positions[sell_code]
        if sell_pos.entry_date == self.trade_date:
            logger.info(f"模拟盘换仓: {sell_code} 当日买入T+1锁定，跳过换仓")
            return False
        sell_price = sell_pos.current_price or price
        logger.info(
            f"模拟盘换仓: 卖出 {sell_code} {sell_pos.stock_name}@{sell_price:.2f} → 买入 {code} {name}"
        )
        ...
        return self.try_buy(
            ...
            source,
            max_amount,
            sector=candidates[0].get("sector", "") if candidates else "",
        )
    ...
    def evaluate_swaps(
        ...
        result = self._ai_evaluate_swap(
            candidates, market_context, price_info, sector_context
        )
        if not result:
            return False
...
只回复JSON：{{"sell": "要卖的代码", "buy": "要买的代码"}} 或 {{"sell": null, "buy": null}}。"""
    ...
    def _rule_swap_target(self, new_code: str, new_score: float) -> str | None:
        ...
        best_priority = 99
        for code, pos in self.portfolio.positions.items():
            if pos.entry_date == self.trade_date:
    ...
    def close(self, code: str, price: float, reason: str):
        ...
        if pos.entry_date == self.trade_date:
            logger.warning(f"T+1 保护，拒绝卖出当日买入的 {code}")
            return

        # 卖出佣金 + 印花税
        amount = price * pos.volume
        commission = (
            max(amount * settings.DEFAULT_COMMISSION_RATE, settings.MIN_COMMISSION) + amount * settings.STAMP_TAX_RATE
        )
        ...
        if self.telegram:
            self.telegram.send(
                ...
                f"   价格: {price:.2f} × {pos.volume} 股  成本: {pos.avg_cost:.2f}  盈亏: {pnl:+,.0f} ({pnl_pct:+.1f}%)\n"
    ...
    def _portfolio_summary(self) -> str:
        ...
        return (
            ...
            f"现金 {p.cash:.0f}  "
            f"总盈亏 {p.total_pnl:+.0f}({p.total_pnl / settings.PAPER_INITIAL_CAPITAL * 100:+.1f}%)"
        )
