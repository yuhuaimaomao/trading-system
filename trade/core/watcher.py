...
from trade.sector.sector_resonance import (
...
class Watcher(
    ...
    def __init__(self, telegram_bot=None, qmt_quote=None, db_path=None):
        ...
        self._sector_trend_history: dict[str, list[float]] = defaultdict(lambda: deque(maxlen=100))
        ...
        # 概念趋势跟踪
        self._concept_trend_history: dict[str, list[float]] = defaultdict(lambda: deque(maxlen=100))
    ...
    def build_state(self) -> ScanState:
        ...
        注意：传引用而非拷贝（性能优先），调用方不得直接修改 dict/list 内容。"""
        return ScanState(
    ...
    def _load_morning_sector_bias(self):
        ...
        if not settings.MORNING_SECTOR_BIAS_ENABLED:
            return
        try:
            rows = self.repo.get_morning_sector_bias(self._trade_date)
            self._morning_sector_bias = {}
            for r in rows:
                self._morning_sector_bias[r["sector_name"]] = {
                    "bias": r["bias"],
                    "priority": r["priority"],
                    "size_mult": r["size_multiplier"],
    ...
    def _resolve_name(self, code: str) -> str:
        ...
        if self.repo is None:
            return code
        name = self.repo.resolve_name(code)
        self._name_cache[code] = name
        if len(self._name_cache) > 1000:
    ...
    def run(self):
        ...
        ai.start_worker()
        self._ai_queue.start()  # 保持兼容（内部也委托给 ai_service）
        self._load_morning_sector_bias()
    ...
    def _cleanup_session_state(self):
        ...
        self._alerted_sl_tp.clear()
        self._sl_reminders.clear()
        self._pullback_alerted_today.clear()
        self._recently_sold.clear()
    ...
    def _scan(self):
        ...
        构建 ScanState 快照传递给各领域模块（引用传递，避免拷贝开销）。
        """
        ...
        try:
            if state.scan_count % 3 == 0 and state.market_snapshot:
        ...
        try:
            self._check_positions(state, prices)
        except Exception as e:
            logger.warning(f"持仓检查异常: {e}", exc_info=True)

        # 主动退出检查
        try:
            self._check_stale_positions(state, prices)
        except Exception as e:
        ...
        try:
            self._check_signals(state, prices, self._regime)
        except Exception as e:
            logger.warning(f"信号检查异常: {e}", exc_info=True)

        try:
            self._check_bought_signals(state, prices)
        except Exception as e:
            logger.warning(f"已买入信号检查异常: {e}", exc_info=True)

        # 引擎2 定期清理过期状态
        if state.scan_count % 30 == 0:
            self._scout_cleanup_stale()

        try:
            self._check_review_picks(state, prices, self._regime)
        except Exception as e:
        ...
        try:
            if state.scan_count % 50 == 0 and state.market_snapshot:
                res_labels = {}
                if len(state.index_prices) >= settings.RESONANCE_INDEX_MIN_POINTS:
                    try:
                        result = self._resonance_analyzer.analyze(
                            index_prices=state.index_prices,
                            ...
                            concept_stats=state.concept_stats,
                            market_snapshot=state.market_snapshot,
                            industry_cache=state.industry_cache,
                            concept_cache=state.concept_cache,
                            trend_starts={
        ...
        try:
            ...
            logger.warning(f"异动/追涨异常: {e}", exc_info=True)

        try:
            if state.scan_count % 15 == 0:
                self._evaluate_swaps(prices)
        except Exception as e:
            logger.warning(f"换仓评估异常: {e}", exc_info=True)

        try:
            self._alert_index_divergence()
        except Exception as e:
            logger.warning(f"指数背离告警异常: {e}", exc_info=True)
        ...
        try:
            if state.scan_count % 5 == 0:
                self._health_check(state, prices)
        except Exception as e:
    ...
    def _health_check(self, state, prices: dict[str, float]):
        """每 5 轮运行所有注册的校验函数，异常推 Telegram。"""
        from trade.core.health_checks import CheckContext, run_checks
        ...
        try:
            pending = self.repo.get_pending_signals(state.trade_date, account="paper")
        ...
        ctx = CheckContext(
            ...
            prices=prices,
            limit_cache=state.limit_cache,
            index_prices=state.index_prices,
            index_high=state.index_high,
            index_low=state.index_low,
            index_close_high=state.index_close_high,
            index_close_low=state.index_close_low,
            index_pre_close=index_quote.get("pre_close", 0),
            qmt_change_pct=index_quote.get("change_pct"),
            sector_stats=state.sector_stats,
            pos_meta=state.pos_meta,
            bought_watch=state.bought_watch,
            sl_reminder_count=len(state.sl_reminders),
            alerted_sl_tp_count=len(state.alerted_sl_tp),
            triggered_ids_count=len(state.triggered_ids),
    ...
    def _restore_pos_meta(self):
        """从持仓表恢复 _pos_meta（止损止盈板块等决策数据）。

        优先使用 trade_portfolio_positions 表中持久化的 stop_loss/take_profit；
        fallback 到 trade_signals 表（兼容旧数据）。
        """
        try:
            for code in self.paper_account.positions:
                pos = self.paper_account.positions[code]
                # 优先从持仓对象读取持久化的 sl/tp
                pos_sl = getattr(pos, "stop_loss", 0) or 0
                pos_tp = getattr(pos, "take_profit", 0) or 0

                if pos_sl > 0 or pos_tp > 0:
                    self._pos_meta[code] = {
                        "sl": pos_sl,
                        "tp": pos_tp,
                        "trailing_stop": getattr(pos, "trailing_stop", 0.05) or 0.05,
                    ...
                    # fallback: 从 trade_signals 恢复（兼容旧持仓数据）
    ...
    def _init_bought_watch(self):
        ...
        codes = list(self.paper_account.positions.keys())
        buy_dates = self.repo.get_buy_dates(codes) if codes else {}
    ...
    def _get_realtime_prices(self, stock_codes: list[str]) -> dict[str, float]:
        """获取实时价格 — 优先从 Collector 推送的 _market_snapshot 取（TCP推送，永不阻塞）。

        QMT HTTP 仅用于获取 preClose 填 _limit_cache，5s 超时不影响价格获取。
        """
        snapshot = getattr(self, "_market_snapshot", None) or {}
        now_ts = time.time()
        prices: dict[str, float] = {}
        need_limits: list[str] = []
        ...
        for code in stock_codes:
            ...
            if item:
                ...
                if price_f > 0:
                    ...
                    # 涨跌停缓存缺失时标记（只取 pre_close，不取行情）
                    if code not in self._limit_cache:
                        need_limits.append(code)

        # preClose 补充查询（5s 超时，不影响主流程）
        if need_limits and self.qmt:
            try:
                quotes = self.qmt.get_realtime(need_limits)
            except Exception:
                quotes = {}
            for code in need_limits:
                item = quotes.get(code) if isinstance(quotes, dict) else None
                if item:
    ...
    def _get_index_ma60(self) -> float:
        ...
        if self.repo is None:
            return 0
        return self.repo.get_index_ma60()

    def _calc_volume_trend(self) -> float:
        """计算近5天全市场成交额趋势。正=放量，负=缩量。"""
        if self.repo is None:
            return 0
        return self.repo.get_volume_trend()
    ...
    def _restore_market_from_db(self):
        ...
        try:
            latest_ts = self.repo.get_latest_market_ts(self._trade_date)
            if not latest_ts:
                return
            rows = self.repo.get_market_snapshots_batch(self._trade_date, latest_ts)
            if not rows:
                return

            self._market_snapshot = {}
            for r in rows:
                self._market_snapshot[r["code"]] = {
    ...
    def _submit_scenario_ai(self, key: str, scenario: str, **fields) -> bool:
        """使用场景模板提交 AI 异步评估。"""
        from system.ai import ai
        from trade.scenario.templates import build_prompt
    ...
    def _ai_chase_opinion(
        ...
        *,
        ...
        if ok:
            ...
            self._alert_private(f"⚠️ AI 队列已满，追高评估跳过 {code} {name}\n   规则决策不受影响")
        return ""
    ...
    def _alert_index_divergence(self):
        """检测多指数背离 + 日内高位回落，推送告警。"""
    ...
    def _check_sector_sharp_move(self):
        ...
        # 查领涨/领跌股（按板块方向过滤，板块跌时只取下跌股）
        down_leaders = self._get_sector_leaders(direction="down") if hasattr(self, "_get_sector_leaders") else {}
        up_leaders = self._get_sector_leaders(direction="up") if hasattr(self, "_get_sector_leaders") else {}

        alerts = []
        for name, s in stats.items():
            chg = s.get("change_pct", 0)
            if chg < -1.5:
                leaders_in_sector = down_leaders.get(name, [])
                label = "领跌"
                marker = "🔴" if name in held_sectors else "⬇"
            elif chg > 2.5:
                leaders_in_sector = up_leaders.get(name, [])
                label = "领涨"
                marker = "🟢"
            else:
                continue
            ...
            if leaders_in_sector:
                stocks = [f"{c[0]} {c[1]} {c[2]:+.1f}%" for c in leaders_in_sector[:3]]
                leader_str = f"\n     {label}: " + ", ".join(stocks)

            if chg < -1.5 or chg > 2.5:
                alerts.append(f"{marker} {name}: {chg:+.1f}%  涨跌{s.get('up', 0)}/{s.get('down', 0)}{leader_str}")
    ...
    def _get_sector_leaders(self, *, direction: str = "") -> dict[str, list[tuple]]:
        """从 market_snapshot 中提取每个板块的领涨/领跌股。

        direction: "up" 只取涨的, "down" 只取跌的, "" 都取。
        返回 {sector_name: [(code, name, chg), ...]}。
        """
        ...
        for code, item in snapshot.items():
            ...
            if industry not in by_sector:
                by_sector[industry] = []
            by_sector[industry].append((code, name, chg))
