    ...
    def _eval_regime(pattern: str, change: float, cg_shift: str) -> dict | None:
        if pattern == "one_sided":
            ...
            if cg_shift == "up" and change > 0.5:
                return {
                    "severity": "P1",
                    "desc": f"判 one_sided 但 30min 内反弹 {change:+.2f}%",
                    "deviation": "opposite",
                }
            if abs(change) < 0.2:
                return {
                    "severity": "P2",
                    "desc": "判 one_sided 但后续横盘",
                    "deviation": "unclear",
                }
            ...
            if cg_shift == "down":
                return {
                    "severity": "P1",
                    "desc": f"判 v_reversal 但继续跌 {change:+.2f}%",
                    "deviation": "opposite",
                }
            ...
            if change > 0.5:
                return {
                    "severity": "P1",
                    "desc": f"判 dead_cat 但持续反弹 {change:+.2f}%",
                    "deviation": "opposite",
                }
        elif pattern == "normal":
            if abs(change) < 0.8:
            ...
            if change < -1.5:
                return {
                    "severity": "P0",
                    "desc": f"判 normal 但 30min 内暴跌 {change:+.2f}%",
                    "deviation": "opposite",
                }
        return None
                        ...
                        "evidence": json.dumps(
                            {
                                ...
                                "close": close,
                                "pnl_pct": round(pnl_pct, 2),
                                "reason_filtered": data.get("reason_filtered"),
                            },
                            ensure_ascii=False,
                        ),
                    }
            ...
            findings.append(
                {
                    ...
                    "evidence": json.dumps(
                        {
                            "small_avg": round(small_avg, 2),
                            "large_avg": round(large_avg, 2),
                        },
                        ensure_ascii=False,
                    ),
                }
