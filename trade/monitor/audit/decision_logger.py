...
class DecisionLoggerMixin:
    ...
    def _log_exit_analysis(self, stock_code: str, holding_status: str,
                           market_env: str, sector_trend: str, **extra):
        self._log_decision("exit_analysis", stock_code=stock_code,
            holding_status=holding_status, market_env=market_env,
            sector_trend=sector_trend, **extra)

    def _log_swap_eval(self, swap_decision: dict, candidate_codes: list,
                       holding_codes: list, **extra):
        self._log_decision("swap_eval",
            swap_decision=swap_decision, candidate_codes=candidate_codes,
            current_holdings=holding_codes, **extra)

    def _log_sector_alert(self, top_sectors: list, bottom_sectors: list,
                           warnings: list, good: list, **extra):
        self._log_decision("sector_alert",
            top_sectors=top_sectors, bottom_sectors=bottom_sectors,
            warnings=warnings, good=good, **extra)

    def _log_resonance_alert(self, index_direction: str, index_change: float,
                              resonance_down: list, counter_up: list, **extra):
        self._log_decision("resonance_alert",
            index_direction=index_direction, index_change=index_change,
            resonance_down=resonance_down, counter_up=counter_up, **extra)
