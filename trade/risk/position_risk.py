...
class PositionRiskMixin:
    ...
    def _check_positions(self, state, prices: dict[str, float]):
    ...
    def _check_stale_positions(self, state, prices: dict[str, float]):
        """主动退出：在硬止损触发前识别该走的仓位。
    ...
    def _check_bought_signals(self, state, prices: dict[str, float]):
        """监控已买入持仓：盯盘状态 + 补仓信号.止损止盈由 _check_positions 统一处理."""
        try:
            rows = self.repo.get_bought_signals_with_entry()
