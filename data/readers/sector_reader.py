...
class SectorReader:
    ...
    @staticmethod
    def get_hot_sectors(conn, trade_date: str, sector_table: str, top_n: int = 10,
                        prev_date: str = None, prev_prev_date: str = None) -> list:
        """
        ...
        if all_sc:
            ...
            """, [trade_date] + all_sc).fetchall()
            chain_count_map = {row['sector_code']: row['chain_count'] for row in chain_rows}
        ...
        for s in sectors:
            ...
            s['turnover_share'] = (s.get('sector_turnover') or 0) / total_market_turnover
        ...
        def rank_pct(key):
            ...
            for s in sectors:
                s[key + '_pct'] = rank_map[s['sector_code']] / (n - 1) * 100 if n > 1 else 0
        ...
        for s in top_sectors:
            ...
            result.append({
                ...
                'up_count': up,
                'down_count': down,
                'up_down_ratio': round(up / down, 1) if down > 0 else (up if up > 0 else 0),
        ...
        if prev_date and prev_prev_date:
            ...
            """, codes + [prev_prev_date, prev_date, trade_date]).fetchall()

            seq_map = {}
            for row in seq_rows:
                seq_map.setdefault(row['sector_code'], {})[row['trade_date']] = \
                    round(row['change_percent'] or 0, 2)
        ...
        if top_codes:
            # 板块 MA5/MA20
            sector_ma_map = SectorReader._calc_sector_ma(conn, trade_date, top_codes, sector_table)

            # 排名变化（昨日排名 vs 今日排名）
            rank_change_map = {}
            if prev_date:
                # 用同样逻辑获取昨日排名
                prev_cursor = conn.execute(f"""
                ...
                prev_rank_map = {s['sector_code']: i+1 for i, s in enumerate(prev_sorted)}
            ...
            if prev_date and prev_prev_date:
                ...
                """, top_codes + [prev_prev_date, prev_date, trade_date]).fetchall()
                for row in flow_rows:
                    fund_flow_3d_map.setdefault(row['sector_code'], {})[row['trade_date']] = \
        ...
        if all_codes:
            ...
            for s in result:
                ...
                def stock_rank_pct(key):
                    ...
                    for st in stocks:
                        st[key + '_pct'] = rank_map[st['code']] / (n_stocks - 1) * 100 \
                            if n_stocks > 1 else 0
                ...
                for st in stocks:
                    st['stock_score'] = round(
                        ...
                        st['circ_mcap_pct'] * 0.10, 2)

                # 市值排名（用于兜底）
                stocks_by_cap = sorted(stocks, key=lambda x: x['circ_mcap'] or 0, reverse=True)
                ...
                s['stocks'] = sorted(selected.values(), key=lambda x: x['stock_score'], reverse=True)

        return result

    @staticmethod
    def get_sector_stocks(conn, trade_date: str, day_before: str, yesterday: str,
                          sector_codes: list, sector_table: str, label: str) -> list:
        """
        ...
        all_codes = [s['code'] for info in groups_by_code.values() for s in info['stocks']]
        hist_data = {}
        if all_codes:
            ...
            for row in cursor.fetchall():
                code = row['stock_code']
                hist_data.setdefault(code, {})[row['trade_date']] = row['change_pct'] or 0
        ...
        for sc in sector_codes:
            ...
            def board_score(boards):
                if boards >= 4: return 100
                elif boards >= 3: return 90
                elif boards >= 2: return 75
                elif boards >= 1: return 50
                return 0
    ...
    @staticmethod
    def _calc_sector_ma(conn, trade_date: str, sector_codes: list, sector_table: str) -> dict:
    ...
    @staticmethod
    def get_sector_zhongjun(conn, trade_date: str, sector_codes: list,
                             sector_table: str, top_n: int = 5) -> dict:
        """
        为每个板块筛选中军候选（4维打分）
        ...
        for code, hist in stock_hist.items():
            ...
            if turnovers:
                today_data[code]['avg_turnover_5d'] = round(sum(turnovers) / len(turnovers), 2)
        ...
        sector_returns = {}
        sector_hist_rows = conn.execute(f"""
            SELECT sector_code, trade_date, change_percent
            FROM {sector_table}
        ...
        for sc in sector_codes:
            ...
            # --- 流动性排名 ---
            by_turnover = sorted(stocks, key=lambda x: x['avg_turnover_5d'], reverse=True)
            ...
            keep_keys = ['code', 'name', 'mcap', 'change_pct', 'ma5', 'ma10', 'ma20',
                         'boards', 'industry', 'avg_turnover_5d', 'rel_strength',
                         'trend_status', 'score']
            clean_top = []
            for s in top:
                clean = {k: s.get(k, 0) if k != 'name' and k != 'industry' and k != 'trend_status'
                         else s.get(k, '') for k in keep_keys}
