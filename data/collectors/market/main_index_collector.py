...
class MainIndexCollector:
            ...
            try:
                ...
                if len(parts) > 50:
                    index_data = {
                        ...
                        "volume": int(float(parts[6]) * 100) if len(parts) > 6 else 0,  # 手→股
                        "turnover_amount": float(parts[37]) if len(parts) > 37 else 0,  # 成交额（万元）
                    }
                    results.append(index_data)
                    logger.info(f"✅ {name}: {index_data['close']:.2f} ({index_data['change_percent']:+.2f}%)")
                else:
    ...
    def save_to_db(self, data: List[Dict], trade_date: str = None, trade_time: str = None):
        """
        保存到数据库（覆盖当天数据）
        ...
        try:
            ...
            # 删除当天数据（覆盖写入）
            cursor.execute("DELETE FROM index_realtime_data WHERE trade_date = ?", (trade_date,))
            conn.commit()
