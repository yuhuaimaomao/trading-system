...
class SectorStocksCollector(ProxyBaseCollector):
    ...
    def __init__(self, trade_date: str = None, task_mgr = None):
        ...
        self.logger.info(f"成分股采集器初始化完成")
    ...
    def _save_progress(self, sector_code: str, sector_name: str, 
                       sector_type: str, **kwargs):
        """
        保存/更新板块采集进度
    ...
    def _get_sector_list(self, sector_type: str) -> List[Dict]:
        ...
        sectors = [
            {'sector_code': row[0], 'sector_name': row[1]}
    ...
    def _collect_single_sector(self, sector_type: str, sector_code: str, 
                                sector_name: str) -> bool:
        """
        采集单个板块的成分股 (支持断点续传)
        ...
        if total_pages == 0:
            for retry in range(self.MAX_RETRIES):
                ...
                if not proxy_dict:
                    self.logger.warning(f"获取代理失败，等待 2 秒后重试...")
                    time.sleep(2)
                    continue
