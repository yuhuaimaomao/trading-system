...
from system.utils.stock_code_utils import strip_stock_code, normalize_stock_code
...
class LimitPoolCollector:
    ...
    def save_to_db(self, limit_up_data: List[Dict], zhapa_data: List[Dict], limit_down_data: List[Dict], trade_date: str = None):
        """
        保存到数据库（覆盖当天数据）
    ...
    def fetch_and_save(self, trade_date: str = None) -> Dict:
        ...
        try:
            ...
            logger.info(f"✅ {self.__class__.__name__} 采集完成:涨停{sealed}只,炸板{len(zhapa_data)}只,跌停{len(limit_down_data)}只,封板率{seal_rate:.1f}%")
            logger.info("="*60)
            return result
    ...
    def print_summary(self, limit_up_data: List[Dict], limit_down_data: List[Dict], ladder_data: List[Dict]):
        """打印数据统计"""
        print('\n【涨跌停池统计】')
        print(f'{"类型":<8} {"数量":>6}')
        print('-' * 16)
