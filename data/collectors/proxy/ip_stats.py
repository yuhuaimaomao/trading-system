...
class IPStatsManager:
    ...
    def _init_db(self):
        ...
        cursor.execute("CREATE INDEX IF NOT EXISTS idx_collector ON ip_usage(collector_name)")
        
        conn.commit()
        conn.close()
    
    def record_usage(self, ip: str, port: int, trade_date: str, 
                     collector_name: str, page: int, status: str, 
                     error: str = None):
        """
        记录 IP 使用情况
        ...
        cursor.execute("""
            ...
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
        """, (ip, port, full_ip, trade_date, collector_name, page, status, error, created_at))
        
        conn.commit()
        conn.close()
    
    def update_ip_detail(self, ip: str, country: str = None, province: str = None, 
                         city: str = None, isp: str = None):
        """
        更新 IP 详情（归属地）
    ...
    def get_ip_usage(self, trade_date: str = None, collector_name: str = None, 
...
def record_ip_usage(ip: str, port: int, trade_date: str,
                   collector_name: str, page: int, status: str,
                   error: str = None):
    """快捷记录 IP 使用"""
