...
try:
    from system.config.proxy_config import (
        ...
        CACHE_DIR, CACHE_ENABLED, CACHE_EXPIRE_HOURS,
    )
...
from data.collectors.proxy.proxy_requester import ProxyRequester, UA_PROFILES, USER_AGENTS
...
class ProxyBaseCollector(ProxyRequester):
    ...
    def __init__(self, logger_name: str = "ProxyBaseCollector",
                 trade_date: str = None, task_mgr = None):
    ...
    def _fetch_page(self, page: int, proxy: Dict) -> Optional[Dict]:
        ...
        try:
            ...
            s.get("https://quote.eastmoney.com/", headers=warmup_headers,
                  proxies=proxy, impersonate=impersonate, timeout=8)
        except Exception:
            pass  # 预热失败不影响主流程

        try:
            data = self._request(
                self.API_URL, params, headers, proxy,
                impersonate, REQUEST_TIMEOUT, session=s,
            )
        finally:
            s.close()
    ...
    def fetch_all(self, max_retries: int = MAX_RETRIES) -> Optional[List[Dict]]:
        ...
        while True:
            ...
            # 如果数据为空，说明已是最后一页
            if not diff_list:
                self.logger.info(f"✅ 已是最后一页（API 返回空数据），采集完成")
                break
            ...
            if page == 1 and data_section.get('total') is not None:
                ...
                if total <= 0:
                    self.logger.info(f"📊 共 0 条数据，采集完成")
                    break
                ...
                # 如果只有 1 页，直接停止
                if total_pages == 1:
                    self.logger.info(f"✅ 只有 1 页，采集完成")
                    break
            ...
            if len(diff_list) < self.PAGE_SIZE:
                self.logger.info(f"✅ 已是最后一页，采集完成")
                break
    ...
    def _load_cache(self) -> Dict:
        ...
        if not CACHE_ENABLED:
            return {'trade_date': self.trade_date, 'status': 'incomplete', 'total_pages': 0, 'completed_pages': 0, 'failed_pages': [], 'updated_at': '', 'data': []}
        ...
        try:
            ...
            self.logger.info(f"✅ 读取缓存成功：{cache.get('completed_pages', 0)}页，{len(cache.get('failed_pages', []))}页失败")
            return cache
