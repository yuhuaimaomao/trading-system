...
class ProxyManager:
    ...
    def __init__(self, trade_date: str = None, collector_name: str = None):
        ...
        self.logger.debug(f"代理管理器初始化: collector={self.collector_name} timeout={PROXY_TIMEOUT}s")
    ...
    def test_proxy(self, proxies: Dict[str, str], test_url: str = "https://www.baidu.com") -> bool:
        """
        测试代理 IP 是否可用
        ...
        try:
            ...
            if response.status_code == 200:
                self.logger.debug(f"✅ 代理 IP 测试成功")
                return True
            else:
