...
from data.repo.repo_base import BaseRepository, _cols_from_str, _validate_cols
...
class StrategyRepo(BaseRepository):
    ...
    def get_funnel_records(self, push_date: str) -> list[dict]:
        cols = _cols_from_str(_FUNNEL_ALL_COLS.replace(" ", ""))
        with self._conn() as conn:
            rows = conn.execute(
                f"SELECT {_FUNNEL_ALL_COLS} FROM strategy_funnel "
                "WHERE push_date=? ORDER BY rank_position",
                (push_date,),
            ).fetchall()
    ...
    def get_ai_decisions(self, push_date: str, verdict: str = None) -> list[dict]:
        cols = _cols_from_str(_AI_DECISION_ALL_COLS.replace(" ", ""))
        with self._conn() as conn:
            if verdict:
                rows = conn.execute(
    ...
    def insert_improvement(self, imp_dict: dict) -> int:
        return self._insert(
            "strategy_improvements", imp_dict, frozenset(imp_dict.keys())
        )

    def get_pending_improvements(self) -> list[dict]:
        cols = _cols_from_str(_IMPROVEMENT_ALL_COLS.replace(" ", ""))
        with self._conn() as conn:
            rows = conn.execute(
                f"SELECT {_IMPROVEMENT_ALL_COLS} FROM strategy_improvements "
    ...
    def apply_improvement(self, imp_id: int):
        with self._conn() as conn:
            conn.execute(
                "UPDATE strategy_improvements SET status='applied', applied_date=? "
                "WHERE id=?",
