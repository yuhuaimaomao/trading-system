...
市场基础数据 — 行业/概念板块排行、资金流、板块个股明细、概念补充。
跨域共享，所有业务线均可使用。
不做格式化、不做判断，纯数据查询。
"""
...
class SectorReader:
    ...
    def get_hot_sectors(
        ...
        if prev_date and prev_prev_date:
            ...
            seq_map = {}
            for row in seq_rows:
                seq_map.setdefault(row["sector_code"], {})[row["trade_date"]] = round(row["change_percent"] or 0, 2)
        ...
        if top_codes:
            ...
            if prev_date:
                ...
                prev_sorted = sorted(
                    [dict(zip([d[0] for d in prev_cursor.description], row)) for row in prev_cursor.fetchall()],
            ...
            if prev_date and prev_prev_date:
                ...
                for row in flow_rows:
                    fund_flow_3d_map.setdefault(row["sector_code"], {})[row["trade_date"]] = round(
                        (row["main_force_net"] or 0) / 100000000, 2
                    )
        ...
        if all_codes:
            ...
            for s in result:
                ...
                def stock_rank_pct(key):
                    ...
                    for st in stocks:
                        st[key + "_pct"] = rank_map[st["code"]] / (n_stocks - 1) * 100 if n_stocks > 1 else 0
    ...
    def get_sector_stocks(
        ...
        for sc in sector_codes:
            ...
            for s in stocks:
                ...
                s["composite_score"] = change_s * 0.30 + board_s * 0.30 + sl_s * 0.20 + vol_s * 0.10 + cap_s * 0.10
    ...
    @staticmethod
    def get_sector_zhongjun(conn, trade_date: str, sector_codes: list, sector_table: str, top_n: int = 5) -> dict:
        """
        为每个板块筛选中军候选（4维打分：市值30%+流动性25%+趋势25%+相对强度20%）

        Returns:
            {sector_code: [{'code', 'name', 'score', 'mcap', ...}, ...]}
        """
        if not sector_codes:
            return {}
        ...
        for row in stock_rows:
            ...
            today_data[code] = {
                ...
                "boards": row["boards"] or 0,
                "avg_turnover_5d": row["turnover_yi"] or 0,
                "stock_5d_return": 0,
                "rel_strength": 0,
            }
        ...
        for sc in sector_codes:
            ...
            for s in top:
                clean = {
                    k: s.get(k, 0) if k != "name" and k != "industry" and k != "trend_status" else s.get(k, "")
                    for k in keep_keys
                }
