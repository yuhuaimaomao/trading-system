...
class ZTPerformanceProcessor:
    ...
    def run(cls, trade_date: str = None) -> Dict:
        ...
        logger.info(f"昨日涨停表现：{stats['total']}只，平均溢价{stats['avg_premium']}%，"
    ...
    def format_report(self, performance_list: List[Dict]) -> str:
        ...
        if top5:
            ...
            for i, perf in enumerate(top5, 1):
                lines.append(f"  {i}. {perf['stock_name']}({perf['stock_code']}) "
    ...
    def save_to_database(self, performance_list: List[Dict], trade_date: str = None):
        ...
        try:
            ...
            # 检查是否已存在
            cursor.execute("SELECT COUNT(*) FROM yesterday_zt_performance WHERE trade_date = ?", (trade_date,))
            count = cursor.fetchone()[0]
            
            if count > 0:
                logger.info(f"{trade_date} 数据已存在（{count}条），删除旧数据")
                cursor.execute("DELETE FROM yesterday_zt_performance WHERE trade_date = ?", (trade_date,))
            ...
            for perf in performance_list:
                # 映射字段
                limit_up_price = perf['yesterday_close'] * 1.2 if perf['stock_code'].startswith(('688', '300', '301')) else perf['yesterday_close'] * 1.1
    ...
    def print_report(self, performance_list: List[Dict], stats: Dict):
        ...
        for stock in performance_list[:10]:
            print(f"{stock['stock_name']:<12} {stock['yesterday_close']:>8.2f} {stock['today_open']:>8.2f} {stock['premium_rate']:>9.2f}% {stock['status']:>8}")
