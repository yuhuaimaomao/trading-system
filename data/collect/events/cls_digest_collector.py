...
class CLSDigestCollector:
    ...
    def fetch_digest_article(self, article_type: str = "morning") -> Optional[Dict]:
        ...
        try:
            ...
            if article_type == "morning":
                ...
                article_id = self._find_latest_article(
                    "早间新闻精选", search_type="all"
                )
                article_name = "早间新闻精选"
            ...
            title = (
                title_match.group(1).strip() if title_match else f"财联社{article_name}"
            )
            ...
            if paragraphs:
                ...
                for p in paragraphs:
                    p = p.strip()
                    if len(p) > 20 and not any(
                        x in p for x in ["举报", "©2018", "版权所有", "许可证"]
                    ):
                ...
                if title_pos >= 0:
                    ...
                    content = re.sub(
                        r"^\s*\d{4}年\d{2}月\d{2}日\s+\d{2}:\d{2}:\d{2}", "", content
                    )
                    content = content.strip()
                    # 按序号分割成段落（1、2、3...）
                    content_lines = re.split(r"(?=\d+、)", content)
                    content_lines = [
                        line.strip() for line in content_lines if len(line.strip()) > 10
                    ]
                    content = "\n".join(content_lines)
                else:
            ...
            if publish_date:
                ...
                date_part = (
                    ...
                    if " " in publish_date
                    else publish_date[:10]
                )
                current_date = datetime.now().strftime("%Y-%m-%d")
                if date_part != current_date:
                    self.logger.warning(
                        f"⚠️ 文章日期不匹配：文章日期={date_part}, 当前日期={current_date}"
                    )
                    return None
            else:
            ...
            if content:
                self.logger.info(
                    f"成功抓取{article_name}文章：{title[:50]}... ({len(content)}字)"
                )
                return {
                    "title": title,
                    "content": content,
    ...
    def _find_latest_article(
        self, keyword: str, search_type: str = "depth"
    ) -> Optional[str]:
        """
        ...
        try:
            ...
            search_url = (
                f"https://www.cls.cn/searchPage?keyword={keyword}&type={search_type}"
            )
            self.logger.info(f"打开搜索页面：{search_url}")

            subprocess.run(
                [agent_browser, "open", search_url], capture_output=True, timeout=15
            )
            ...
            result = subprocess.run(
                [agent_browser, "snapshot"], capture_output=True, text=True, timeout=10
            )
            ...
            if article_id:
                ...
                self.logger.warning("未从快照中解析到文章 ID")
                return None
    ...
    def _parse_article_id_from_snapshot(
        self, snapshot: str, keyword: str
    ) -> Optional[str]:
        """
        从快照中解析文章 ID
                    ...
                    if title_match:
                        ...
                        if keyword in title:
                            ...
                            for j in range(i, min(i + 3, len(lines))):
                                url_match = re.search(
                                    r"/url:\s*/detail/(\d+)", lines[j]
                                )
                                if url_match:
                                    article_id = url_match.group(1)
                                    self.logger.info(
                                        f"解析到文章 ID: {article_id}（标题：{title[:50]}）"
                                    )
                                    return article_id
        ...
        try:
            ...
            if title_pos >= 0:
                ...
                date_match2 = re.search(
                    r"(\d{4})年(\d{1,2})月(\d{1,2})日\s*(\d{2}:\d{2})", context
                )
                if date_match2:
