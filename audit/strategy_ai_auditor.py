...
class AIAuditor:
    ...
    def __init__(self, db_path: str = None, repo=None):
        self.repo = repo or TradeRepository(db_path=db_path)
        self.db_path = db_path

    def review(self, findings: list[dict], context: dict | None = None) -> dict:
        """审计管线统一接口 — 委托给 audit()。"""
        ctx = context or {}
        push_date = ctx.get("date", "")
        result = self.audit(push_date, findings)
        if not result:
    ...
    def _call_ai(self, prompt: str) -> Optional[str]:
        ...
        if not hasattr(ai, "chat"):
            logger.warning("AI 服务未配置，跳过 AI 审计")
            return None
