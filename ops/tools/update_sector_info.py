...
def update_sector_info_daily(trade_date: str = None):
    ...
    try:
        ...
        """)
        existing_sectors = {
            row["sector_code"]: row["need_collect"] for row in cursor.fetchall()
        }
        ...
        for sector in current_sectors:
            ...
            if code not in existing_sectors:
                ...
                logger.info(
                    f"  ✅ 新增：{code} {name} ({reason}) → need_collect={need_collect}"
                )
                new_sectors_added += 1
        ...
        # 查询更新后的统计
        cursor.execute(
            "SELECT COUNT(*) as count FROM sector_info WHERE need_collect = 0"
        )
        event_sectors = cursor.fetchone()["count"]

        cursor.execute(
            "SELECT COUNT(*) as count FROM sector_info WHERE need_collect = 1"
        )
