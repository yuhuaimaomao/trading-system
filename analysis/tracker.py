...
from system.config.settings import PROJECT_ROOT, DATABASE_PATH
...
class StockTracker:
    ...
    # 推荐理由关键词
    REASON_KEYWORDS = [
        '反包', '突破', '主力净流入', '涨停', '龙头', '趋势',
        '放量', '缩量', '支撑', '压力', '缠论', '一买', '二买', '三买',
        '洗盘', '拉升', '建仓', '主升浪', '连板', '首板',
        '强势', '弱势', '震荡', '回调', '反弹'
    ]
    ...
    def _create_excel_sheet(self, from_scratch: bool = True):
        ...
        # 设置表头（23 列）
        headers = [
            '推送日期', '股票名称', '股票代码', '所属板块', '预期强度',
            '真实市值 (亿)', '推荐理由', '开盘表现', '收盘表现', '日内差',
            '是否涨停', '次日开盘表现', '次日开盘收益', '开盘胜率判定', '次日收盘收益',
            '收盘胜率判定', '次日高点收益', '高点胜率判定', '次日低点收益', '平均股价收益率',
            '平均股价胜率判定', '备注', '来源'
        ]
        ...
        # 设置列宽
        column_widths = [12, 12, 10, 15, 10, 12, 20, 12, 12, 12, 10, 12, 12, 10, 12, 10, 12, 10, 12, 15, 15, 15, 8]
    ...
    def enrich_stock_pool(self, stock_pool: list) -> list:
        ...
        for stock in stock_pool:
            ...
            if ai_market_cap:
                ...
                # AI 没提供市值，查询数据库
                market_cap = self.get_market_cap(stock_code, push_date)
                logger.info(f"🔍 AI 未提供市值，查询数据库：{stock_name} - {market_cap:.1f}亿" if market_cap else "查询失败")
        ...
        for line in lines:
            ...
            if '🔥' in line and '强度' in line:
                ...
                if start != -1 and end != -1:
                    ...
                    stars = star_content.replace('(', '').replace(')', '').replace('（', '').replace('）', '').replace('强度', '').replace(':', '').replace(':', '').strip()
            ...
            if current_plate and '【' in line:
                ...
                for stock_name in matches:
                    ...
                    if not stock_code:
                        ...
                        logger.error(f"   跳过该股票，不记录到追踪表")
                        logger.error(f"   可能原因：股票名称错误/今日停牌/数据缺失")
                        continue
    ...
    def record_stocks(self, stocks: list, push_date: str, brief_text: str = '', source: str = '早报'):
        ...
        for stock in stocks:
            ...
            if key not in existing_keys:
                self.ws.append([
                    push_date, stock.get('股票名称', ''), code,
                    stock.get('所属板块', ''), stock.get('预期强度', ''),
                    stock.get('真实市值', ''), stock.get('推荐理由', ''),
                    '', '', '', '', '', '', '', '', '', '', '', '', '', '', '',
                    source,
                ])
                row = self.ws.max_row
    ...
    def update_daily_data(self, trade_date: str):
        ...
        """
        from system.config.trading_calendar import get_next_trading_day, get_previous_trading_day
        ...
        if prev_day:
            ...
            """, (prev_day,))
            catch_up_rows = cursor.fetchall()

        if not rows and not catch_up_rows:
        ...
        for r in rows:
            if r['source'] == '复盘':
                records.append({'stock_code': r['stock_code'], 'push_date': trade_date,
                                'target_date': next_trade_date, 'source': '复盘'})
            else:
                records.append({'stock_code': r['stock_code'], 'push_date': trade_date,
                                'target_date': trade_date, 'source': '早报'})
        for r in catch_up_rows:
            records.append({'stock_code': r['stock_code'], 'push_date': prev_day,
                            'target_date': trade_date, 'source': '复盘追赶'})

        morning_count = sum(1 for r in records if r['source'] == '早报')
        review_count = sum(1 for r in records if r['source'] == '复盘')
        logger.info(f"需要更新 {len(records)} 只（早报:{morning_count} 复盘:{review_count}"
                    f" 追赶:{len(catch_up_rows)}）")
        ...
        for target_date, recs in by_target_date.items():
            ...
            for row in cursor.fetchall():
                ...
                market_data[key] = {
                    'open': row['open'], 'close': row['price'],
                    'prev_close': row['prev_close'], 'change_pct': row['change_pct'],
                }
            cursor.execute(f"""
        ...
        for r in records:
            ...
            try:
                ...
                """, (
                    data['open'], data['close'], data['prev_close'],
                    data['change_pct'], t_open_pct, t_intra_diff,
                    is_limit_up, r['push_date'], code
                ))
                if cursor.rowcount > 0:
                    update_count += 1
            except Exception as e:
    ...
    def update_next_day_data(self, yesterday: str, today: str):
        ...
        以避免同股票跨日重复时覆盖遗漏。
        """
        from system.config.trading_calendar import get_previous_trading_day, get_next_trading_day
        ...
        for key, info in yesterday_stocks.items():
            ...
            try:
                cursor.execute("""
                    ...
                    WHERE push_date = ? AND stock_code = ?
                """, (
                    t1_open, t1_open_pct, t1_avg_price,
                    final_return,
                    t1_close_return, t1_high_return, t1_low_return,
                    avg_price_return,
                    push_date, code
    ...
    def _update_excel_daily_data_v2(self, trade_date: str, records: list, market_data: dict,
                                     limit_up_stocks: set):
        """
