...
class AIAdvisor:
    ...
    def analyze(
        ...
        with ThreadPoolExecutor(max_workers=len(self._analyzers)) as executor:
            future_map = {
                executor.submit(
                    self._call_and_parse, name, analyzer, prompt, trade_date
                ): name
                ...
                try:
                    ...
                    if result:
                        ...
                        if signals:
                            ...
                            logger.info(
                                f"{name} 分析完成: 生成 {len(signals)} 个信号, "
