...
class CollectionStatsService:
    ...
    # A 类采集器：只显示实际数量
    A_CLASS = ['lhb', 'limit_pool', 'stock_monitor', 'regulatory_letter', 'strong_stock', 'share_holder_change', 'suspend_resume']
    ...
    def generate_summary(self, data: Dict[str, Any], 
        ...
        for name, value in data.items():
            ...
            if name in self.A_CLASS:
                ...
                summary.append(f"{status} {cn_name}: 应采集{expected}条，实际{actual_count}条")
                ...
                if isinstance(value, dict) and not value.get('success'):
                    ...
                    summary.append(f"{status} {cn_name}: 应采集{total_count}个，实际{actual_count}个")
                    
                    if actual_count < total_count:
                        failed_items.append(f"{cn_name}: 缺{total_count - actual_count}个")
    ...
    def check_and_report(self, data: Dict[str, Any], 
                         trade_date: str = None,
                         send_to_telegram: bool = True) -> str:
        """
        检查数据并生成报告
