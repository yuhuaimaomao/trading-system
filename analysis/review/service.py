...
from data.collectors.events.share_holder_change_collector import ShareHolderChangeCollector
...
class ReviewService:
    ...
    def collect(self):
        ...
        # 确保所有采集器使用正确的交易日期
        for col in [self.industry_collector, self.concept_collector,
                     self.stock_basic_collector, self.strong_stock_collector,
                     self.suspend_resume_collector, self.share_holder_change_collector]:
        ...
        try:
            ...
            regulatory_letter_result = {'success': False, 'count': 0, 'total': 0, 'data': []}
        ...
        try:
            ...
            stock_monitor_result = {'success': False, 'count': 0, 'total': 0, 'data': []}
        ...
        try:
            ...
            suspend_resume_result = {'success': False, 'count': 0, 'total': 0, 'data': []}
        ...
        try:
            ...
            share_holder_change_result = {'success': False, 'count': 0, 'total': 0, 'data': []}
    ...
    def _check_data_complete(self, trade_date: str) -> list:
        ...
        missing = []
        checks = {
            '个股行情': ("SELECT COUNT(*) FROM stock_basic WHERE trade_date = ?", 'stock_basic'),
            '行业板块': ("SELECT COUNT(*) FROM sector_industry WHERE trade_date = ?", 'sector_industry'),
            '概念板块': ("SELECT COUNT(*) FROM sector_concept WHERE trade_date = ?", 'sector_concept'),
        }
