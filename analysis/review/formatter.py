...
def format_candidates(candidates: list, sector_code_map: dict = None) -> str:
    ...
    categories = {'[涨停驱动]': [], '[资金异动]': [], '[强势突破]': [], '[龙虎榜加持]': []}
    ...
    for cat, recs in categories.items():
        ...
        if len(recs) > 8:
            lines.append(f"    ...等{len(recs)-8}只（调用 get_unusual_stocks 查全量）")
    return "\n".join(lines)
...
def format_strong_stocks(records: list, sector_code_map: dict = None) -> str:
    ...
    for cat, recs in categories.items():
        ...
        for i, r in enumerate(show, 1):
            ...
            extra = []
            if r.get('is_limit_up'): extra.append('涨停')
            if r.get('limit_up_days'): extra.append(f"{r['limit_up_days']}天{r.get('limit_up_count', 0)}板")
            extra_str = f" [{', '.join(extra)}]" if extra else ""
            lines.append(
...
def format_yzt_performance(records: list, sector_code_map: dict = None) -> str:
    ...
    if down_5pct:
        ...
        lines.append(f"  亏钱效应：{len(down_5pct)}只跌超5%，最深{worst['name']}({worst.get('code','')}) {worst.get('change',0):+.2f}%")
    else:
        lines.append(f"  亏钱效应：无跌超5%个股，容错率较高")
    ...
    lines.append(f"  精选{len(kept)}只（连板成功+亏损≤-3%TOP10），其余{skipped}只略"
...
def format_broken_boards(records: list, broken_trend: dict = None, sector_code_map: dict = None) -> str:
    ...
    if broken_trend:
        d2, d1, d = broken_trend.get('d2', 0), broken_trend.get('d1', 0), broken_trend.get('d', 0)
    ...
    for ind, recs in sorted_groups:
        ...
        if len(recs) > 5:
            ...
            lines.append(f"    ...等{len(recs)-5}只：{'、'.join(rest_names[:10])}"
...
def format_lhb_full(lhb_data: list) -> str:
    ...
    for s in lhb_data:
        ...
        hm_count_local = sum(1 for seat in buy_seats
                             if any(k in (seat.get('name') or '') for k in
                                    ('国泰海通', '华泰', '华鑫', '中信', '银河', '国盛')))
...
def calc_position_cap(limit_up: int, broken: int, highest_board: int,
                      seal_rate: float, up_ratio: float,
                      yzt_avg_change: float = 0) -> int:
    """加权评分计算仓位硬顶（0-100分 → 仓位）"""
