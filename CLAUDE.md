...
```
  ...
  │                                                                   │
  │  T 9:24 cron: monitor → 等到 9:30 → 盯盘直到 15:00                │
  │         │                                                        │
  │         ├─ [第1轮 9:30] _send_opening_decision() 开盘决策汇总      │
  │         │                                                        │
  │         ├─ [每轮 60s]                                            │
...
│   │   ├── abnormal.py         #     异动检测
│   │   └── audit/              #     盯盘自审计子系统
│   │       ├── decision_logger.py    #  决策日志 Mixin
│   │       ├── rule_auditor.py       #  RuleAuditor 规则引擎
│   │       ├── ai_auditor.py         #  AIAuditor AI审计
│   │       ├── prompts.py            #  AI 审计 prompt
│   │       └── improvement_applier.py#  改进建议应用器
│   ├── execution/              #   执行层
...
# 盘中
python main.py monitor             #  9:24 cron → 9:30 启动盯盘，自管理生命周期到 15:00
...
StrategyPipeline.run(trade_date)
  ...
  │   ├─ AI 分析 → JSON 格式(v2): 主用 deepseek-v4-pro
  ...
  │   ├─ AI 异常 → fallback DeepSeek(deepseek-v4-pro)
...
4. **AI 模型** — 主用 deepseek-v4-pro（DeepSeek provider），永不使用 deepseek-chat。DashScope key 保留备用。`.env` 设 `AI_MODEL=deepseek-v4-pro` 即可
...
14. **深跌等待反弹** — 亏损 > 7% 且非下午（14:00 前），止损触发后不立即卖出，而是 `continue` 跳过执行等反弹机会。14:00 后评估反弹幅度决定是否尾盘止损。健康检查 `_stop_not_triggered` 已知此逻辑，不误报
15. **智能仓位替代固定比例** — 不再所有票买 10%，而是根据市场模式（0-20000）+ 板块趋势（±20-40%）+ 买入区位置（±10-30%）动态计算
16. **模拟盘费率对齐实际** — 佣金万 0.85 最低 5 元，印花税万分之五（减半征收）卖出单边。avg_cost 含佣金
17. **买入后盯盘不丢** — 买入后每 ~20 分钟推送持仓状态（健康/观察/接近止损/被套/深度套牢/补仓机会 六类），不再像以前买入就不管了
18. **复盘上下文注入 AI** — 盘后复盘报告的结论（情绪周期/主线/次线/退潮/情景推演/仓位建议）注入次日策略管线的 AI prompt，AI 据此调整 confidence 和选股方向
19. **复盘精选统一盯盘** — 所有复盘角色（主线龙头/中军/补涨/次线龙头/趋势票）统一转为结构化 OrderSignal（source=REVIEW），合并到 trade_signals，和 AI 信号用同一套 buy_zone/sl/tp 格式，Watcher 统一盯盘
20. **利润回撤止盈分级 + 大盘加成** — 不是简单的"回撤 X% 就卖"，而是根据最高浮盈分三级 + 大盘 risk_level 加成。极端行情多保留 10% 利润（T1 保留 70%），危险行情多保留 5%。浮盈越大的票给更多回撤容忍空间
...
39. **盯盘自审计闭环**（2026-06-01）— Watcher 每轮关键决策写入 `watcher_decision_log`（regime变化/买入触发-过滤/止损-止盈/仓位/被套分析/换仓/板块预警/共振告警共10种类型）。收盘后 RuleAuditor（纯Python，6维度：市场模式+买入信号+止损+止盈+仓位+板块共振）回溯验证每笔决策 vs 后续实际走势，输出 `audit_findings`。AIAuditor 串联因果链+发现规律+生成 `watcher_improvements`，推送改进卡片到 Telegram。用户回复「应用 #N」标记采纳，下次审计验证效果。决策日志写入失败不影响盯盘主流程。详见 `docs/2026-05-31-watcher-self-audit-design.md`。
40. **AI 多 Provider 支持**（2026-06-01，更新 06-02）— `AIAnalyzer` 根据模型名前缀自动路由（qwen→DashScope, deepseek→DeepSeek OpenAI-compatible）。`settings.AI_MODEL` 不再有默认值，强制 .env 配置。所有代码中无硬编码模型名/Provider 名。`.env` 中设 `AI_MODEL=deepseek-v4-pro` 即可全局切换。**永不使用 deepseek-chat**。向下兼容：`DASHSCOPE_MODEL`/`DASHSCOPE_ANALYSIS_MODEL`/`AUDIT_AI_MODEL` 仍可单独覆盖。
...
- **cron 完整调度**:
  ...
  */5 9-17 * * 1-5 collect --module news → 盘中电报
  24 9 * * 1-5  qmt-collect → QMT 实时数据采集（TCP server，Watcher 依赖）
  24 9 * * 1-5  monitor    → 盯盘进程（9:30开始扫描，9:30推送「📋开盘决策」）
  0  9 * * 0   cleanup    → 周清理
  ```
- **Telegram 推送时间线**:
  ```
  ...
  T    9:00         早盘简报
  T    9:30         📋 开盘决策（集合竞价后汇总：持仓+买入区+待观察+集中度）
  T    9:30-15:00   盘中消息（🔴买入/⚠️止损/✅止盈/📊板块/🏭异动/📈技术拐点...）
  T   15:00         盯盘结束 → 推送收盘持仓报告（模拟盘群聊 + 实盘私聊）
  ```
- **FunctionCallingEngine 工具注册双写 (2026-06-03)**：新增工具时必须同时在两处注册：① `stock_tools.py` 的 `TOOLS_DEFINITION`（AI 可见）② `function_calling.py` 的 `FunctionCallingEngine.tool_functions` 字典（实际执行）。漏了②会导致 AI 调用工具时 `❌ 未找到工具`。`stock_tools.py` 底部的 lazy dict 是复盘系统用的，FC 引擎不用
- **早盘简报 ADJUSTMENTS 清理 (2026-06-03)**：`_remove_adjustments` 加了兜底逻辑——如果 AI 漏掉 `<<<END>>>`，从 `<<<ADJUSTMENTS>>>` 标记处截断文本，防止原始标记泄露到 Telegram
...
- **买卖操作 9:30 后才允许（2026-06-02）**：`MORNING_START = dt_time(9, 30)`，`_in_trading_hours()` 和 `_before_market()` 均以此为界。`_scan()` 循环只在 `_in_trading_hours()` 为 True 时执行，买卖操作自然限制在 9:30 后
