...
40. **AI 多 Provider 支持**（2026-06-01）— `AIAnalyzer` 根据模型名前缀自动路由（qwen→DashScope, deepseek→DeepSeek OpenAI-compatible）。`settings.AI_MODEL` 不再有默认值，强制 .env 配置。所有代码中无硬编码模型名/Provider 名。`.env` 中设 `AI_MODEL=deepseek-chat` 即可全局切换。向下兼容：`DASHSCOPE_MODEL`/`DASHSCOPE_ANALYSIS_MODEL`/`AUDIT_AI_MODEL` 仍可单独覆盖。
