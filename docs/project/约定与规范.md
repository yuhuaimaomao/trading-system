...
项目编码规范、关键约定。持续更新。陷阱/坑见 [已知陷阱.md](已知陷阱.md)。
...
3. **行情优先走 Collector 快照**：`_get_realtime_prices` 优先从 `_market_snapshot`（Collector TCP 推送）取价格，QMT HTTP 仅用于补充涨跌停价（preClose）。禁止用 `self.qmt.get_realtime()` 作为个股价格主源（READ_TIMEOUT=120s，会卡死 Watcher）
